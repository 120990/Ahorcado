...
const diccionario = ["BAÑERA", "SAXOFON", "ARROZ", "CANCHA", "PELOTA"];
...
function clickLetra(event) {
    ...
    if (existenciaLetra) {
        letrabtn.style.cssText = "background-imagenes: url(imagenes/check.png); background-size: cover; text-shadow: 0 0 5px white; -webkit-text-stroke: 0.5px white; color: rgba(10,56,113,0.75);";
        sfx.won.play();
    } else {
        vidasGastadas += 1;
        letrabtn.style.cssText = "background-image: url(imagenes/cruz.png); background-size: cover; text-shadow: 0 0 5px white; -webkit-text-stroke: 0.5px white; color: rgba(10,56,113,0.75);";
        if (vidasGastadas >= 6) {
            letras.forEach(Element => { Element.disabled = true });
        }
        generarDibujo();
...
function pantallaPerdedor() {
    ...
    svg.style.cssText = "background-image: url(imagenes/colorsvg.png); background-size: cover;stroke: #343A40;";
...
function pantallaGanador() {
    ...
    document.getElementById("body").style.cssText = "background-imagenes: linear-gradient(white, rgba(0,128,0,0.2));";
    sfx.goal.play();
    if (ronda == elegirMatriz()) {
        juegoTerminado();
        document.getElementById("desistir").value = "Salir";
